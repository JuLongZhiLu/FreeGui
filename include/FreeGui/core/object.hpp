...
public:
  Object() = default;
  virtual ~Object() { disconnectAllSignals(); }

  template<typename... Args>
  using SignalConnection =
    std::pair<typename Signal<Args...>::SlotID, Signal<Args...>*>;

  template<typename... Args>
  SignalConnection<Args...> connectSignal(
  ...
  template<typename... Args>
  void disconnectSignal(const std::string& name,
                        typename Signal<Args...>::SlotID id)
  {
    std::lock_guard<std::mutex> lock(signalsMutex_);
